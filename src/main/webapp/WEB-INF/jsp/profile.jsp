...
<%@ taglib prefix="coachu" tagdir="/WEB-INF/tags" %>
...
<div class="jumbotron">
    ...
    <div class="container">
        ...
        <h2>${user.name} <spring:message code="${register ? 'app.register' : 'app.profile'}"/></h2>
        ...
        <form:form modelAttribute="user" class="form-horizontal" method="post" action="${register ? 'register' : 'profile'}"
            ...
            <coachu:inputField label='${userName}' name="name"/>

            <spring:message code="user.email" var="userEmail"/>
            <coachu:inputField label='${userEmail}' name="email"/>

            <spring:message code="user.password" var="userPassword"/>
            <coachu:inputField label='${userPassword}' name="password" inputType="password"/>
